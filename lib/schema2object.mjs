...
function validateType(value, schema, path, root) {
  ...
  if (type !== undefined) {
    ...
    if (!types.some(t => TYPE_CHECKS[t]?.(value))) {
      ...
      const hint = types[0] === 'string' ? ` — use "${value}"` :
                   types[0] === 'number' ? ` — use ${Number(value)}` :
                   types[0] === 'boolean' ? ` — use ${value === 'true' || value === true}` :
                   types[0] === 'array' ? ` — use [${JSON.stringify(value)}]` : ''
      throw new TypeError(`${path}: expected ${types.join('|')}, got ${got} (${JSON.stringify(value)})${hint}`)
    }
  }
...
export class ObjectTree {
  ...
  withDefaults() {
    ...
    return new ObjectTree(_applyDefaults({ ...data }, this.#schema, this.#root), this.#root)
  }
...
function _softValidate(data, schema, root) {
  try { validateType(data, schema, '$', root || schema); return true } catch { return false }
}
...
function _applyDefaults(data, schema, root) {
  if (!schema || typeof schema !== 'object') return data
  const resolved = schema.$ref ? _resolveRef(schema.$ref, root || schema) : schema
  const props = resolved.properties
  if (!props || typeof props !== 'object') return data
  for (const [k, rawS] of Object.entries(props)) {
    const s = rawS?.$ref ? _resolveRef(rawS.$ref, root || schema) : rawS
    // Merge: $ref-level default can be overridden by property-level default
    const def = rawS?.default ?? s?.default
    if (Object.prototype.hasOwnProperty.call(data, k)) {
      if (data[k] && typeof data[k] === 'object' && !Array.isArray(data[k]))
        data[k] = _applyDefaults({ ...data[k] }, s, root)
    } else if (def !== undefined) {
      data[k] = typeof def === 'object' ? JSON.parse(JSON.stringify(def)) : def
    }
  }
  return data
}
